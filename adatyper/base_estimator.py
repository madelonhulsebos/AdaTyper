...
class TableMachineLearningEstimator(base.BaseEstimator, base.ClassifierMixin):

    def __init__(self, class_labels):
        super().__init__()
        self.class_labels = class_labels
        self.class_labels_ = None
        self.typetabert = None
        ...
        self.model = typetabert.TypeTaBERTModel(
            config["num_classes"],
            "cpu",
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        "The TypeTaBERT estimator is trained through a separate module (see typetabert/train)."
        print("Fitting ML estimator.")

        # TODO: once fitted, is the typetabert model actually stored in the joblib object?
    ...
    def predict(self, X: pd.DataFrame, threshold):
        """."""
        probs = self._infer_types_typetabert(X)
        y_hat = probs.idxmax(axis=1)
    ...
    def _infer_types_typetabert(self, X: pd.DataFrame):
        # X = X.reset_index(drop=False)
        probs = pd.DataFrame(columns=self.class_labels_, index=np.arange(X.shape[0]))

        for column_encoding in X.iterrows():
            outputs = self.model(column_encoding.to("cpu"))
            predicted_types = self.model.convert_outputs_to_types(outputs)
            predicted_confidences = torch.max(outputs).detach().numpy()

            in_catalog_indices = [
                index
                for index, predicted_type in enumerate(predicted_types)
                if predicted_type in self.class_labels_
            ]

            predicted_types_in_catalog = predicted_types[in_catalog_indices].flatten()
            predicted_confidences_in_catalog = predicted_confidences[in_catalog_indices]
            predicted_indices_in_catalog = table.index[in_catalog_indices].tolist()

            probs.loc[
                predicted_indices_in_catalog,
                predicted_types_in_catalog
            ] = predicted_confidences_in_catalog * 100
